...
from Final_Assignment.quaquel_github_files import funs_generate_network
from Final_Assignment.quaquel_github_files.funs_dikes import Lookuplin, dikefailure, init_node
from Final_Assignment.quaquel_github_files.funs_economy import cost_fun, discount, cost_evacuation
from Final_Assignment.quaquel_github_files.funs_hydrostat import werklijn_cdf, werklijn_inv
import numpy as np
import pandas as pd
import os
...
class DikeNetwork(object):
    def __init__(self):
        ...
        try:
            self.A = pd.read_excel(f'data//hydrology//werklijn_params.xlsx')
        except:
            self.A = pd.read_excel(fr'{self.path}\\quaquel_github_files_kopie\\data\\hydrology\\werklijn_params.xlsx')

...
import pandas as pd
from Final_Assignment.quaquel_github_files.funs_dikes import Lookuplin  # @UnresolvedImport
...
def get_network(plann_steps_max=10):
    ...
    if len(projects.columns) == 6:
        ...
        projects.set_index('project name', inplace=True)
    elif len(projects.columns) == 7:
        projects.columns = ['project name', 0,1,2,3,4,5]
        projects.drop(columns=5, inplace=True)
        projects.set_index('project name', inplace=True)

    else:
        print(f"!!!!!\n"
              f"Dataframe has an unexpected number of columns: {len(projects.columns)}\n"
              f"!!!!!\n\n")

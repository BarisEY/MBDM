...
from Final_Assignment.quaquel_github_files.dike_model_function import DikeNetwork  # @UnresolvedImport
import numpy as np
...
def get_model_for_problem_formulation(problem_formulation_id):
    ...
    # #Our PF:
    elif problem_formulation_id == 30:
        variable_names = []
        variable_names_ = []
        variable_names__ = []
        our_dikelist = np.array(['A.1', 'A.2'])

        for n in function.planning_steps:
            variable_names.extend([f'{dike}_Expected Annual Damage {n}' for dike in our_dikelist])

            variable_names_.extend([f'{dike}_Dike Investment Costs {n}' for dike in our_dikelist] +
                                   [f'RfR Total Costs {n}'] +
                                   [f'Expected Evacuation Costs {n}'])

            variable_names__.extend([f'{dike}_Expected Number of Deaths {n}' for dike in our_dikelist])
